...
trait MathOpsExp extends MathOps with EffectExp {
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = ({
    ...
    e match {
      ...
      case Reflect(MathAbs(x), u, es) => reflectMirrored(Reflect(MathAbs[A](f(x)), mapOver(f,u), f(es)))(using mtyp1[A], pos)
      ...
      case Reflect(MathMin(x,y), u, es) => reflectMirrored(Reflect(MathMin[A](f(x),f(y)), mapOver(f,u), f(es)))(using mtyp1[A], pos)
      case Reflect(MathMax(x,y), u, es) => reflectMirrored(Reflect(MathMax[A](f(x),f(y)), mapOver(f,u), f(es)))(using mtyp1[A], pos)
