...
trait FractionalOpsExp extends FractionalOps with ImplicitOpsExp with EffectExp {
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case e@FractionalDivide(a,b) => fractional_divide(f(a),f(b))(using e.f.asInstanceOf[Fractional[A]],mtype(e.mT),pos)
    case Reflect(e@FractionalDivide(a,b), u, es) => reflectMirrored(Reflect(FractionalDivide(f(a),f(b))(e.f.asInstanceOf[Fractional[A]],mtype(e.mT)), mapOver(f,u), f(es)))(using mtyp1[A], pos)
